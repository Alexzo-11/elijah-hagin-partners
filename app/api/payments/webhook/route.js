...
export async function POST(request) {
  try {
    ...
    // Verify Paystack signature (implement in production)
    ...
    if (event === 'charge.success') {
      ...
      if (data.status === 'success' && payment.status !== 'success') {
        ...
        payment.receiptNumber = `RCP-${Date.now()}-${Math.random().toString(36).substring(7)}`;
        ...
        // TODO: Send confirmation email, create notification, etc.
      }
    }
